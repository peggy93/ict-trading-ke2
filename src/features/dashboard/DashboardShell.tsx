...
import { CHART_TIMEFRAMES } from "@/config/constants";
import type { MarketType, Timeframe } from "@/types";
import { MarketHeader } from "./MarketHeader";
import { OrderBookPanel } from "@/features/orderbook/OrderBook";
import { SignalPanel } from "@/features/signals/SignalPanel";
import { StructurePanel } from "@/features/structure/StructurePanel";
import { LiquidityPanel } from "@/features/structure/LiquidityPanel";
import { SessionTimer } from "@/features/sessions/SessionTimer";
import { VolumePanel } from "@/features/volume/VolumePanel";
import { Watchlist } from "@/features/watchlist/Watchlist";
import { PerformanceMetricsPanel } from "@/features/metrics/PerformanceMetrics";
import { ConnectionBadges } from "@/features/status/ConnectionBadges";
...
const CandleChart = dynamic(
  () => import("@/features/chart/CandleChart").then((m) => m.CandleChart),
  {
    ssr: false,
    loading: () => (
      ...
      <header className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold">
            <span className="text-[var(--accent)]">◆</span> ICT / SMC Scanner
          </h1>
          <ConnectionBadges />
        </div>
